...
https://github.com/user-attachments/assets/forkgpt_compressed.mp4
...
|  **Merge**   | Collapse a branch back into its parent. An LLM generates a summary of only the _unique_ content (shared history is filtered out). |
...
Both `branches` and `messages` tables use **self-referencing foreign keys** (`parent_branch_id` → `branches.id`, `parent_id` → `messages.id`). This creates a tree structure that PostgreSQL's `WITH RECURSIVE` CTEs can traverse in a **single query** — no N+1 problem:
...
2. **Filter** to only messages _after_ `root_message_id` — exclude all shared history.
...
```bash
git clone https://github.com/<your-username>/branchgpt.git
cd branchgpt
npm install
```
...
Open [http://localhost:3000](http://localhost:3000). Click **"Try Demo"** to explore features instantly — no API key required for the demo.
...
│ id            (PK)   │              │ id            (PK)   │
│ name                 │              │ content              │
│ root_message_id (FK→messages.id)   │ role (user|asst|sys) │
│ parent_branch_id (FK→branches.id)  │ parent_id  (FK→self) │
...
- **Primary Accent:** `#2dd4bf` (teal/cyan) — active branches, buttons, neon glows
- **Secondary Accent:** `#a78bfa` (purple) — merge operations, merged branch indicators
- **Glass effects:** Semi-transparent panels with `backdrop-blur` for depth
- **Typography:** Space Grokes (UI) + JetBrains Mono (code), loaded via `next/font` for zero CLS
