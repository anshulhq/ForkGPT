...
export default function ChatPage() {
    ...
    const handleFork = async (messageId: string, newContent: string) => {
        if (demoMode) {
            alert("Forking is disabled in the introductory tour. Start a new chat to explore branching!");
            return;
        }
    ...
    const confirmDelete = async () => {
        ...
        if (branchId.startsWith("demo-")) return;
    ...
    if (!isInitialized) {
        return (
            <div className="flex items-center justify-center h-screen bg-background relative overflow-hidden">
                <div className="absolute inset-0" style={{ background: "radial-gradient(circle at center, rgba(45,212,191,0.04) 0%, transparent 60%)" }} />
                <div className="text-center z-10">
                    <div className="w-10 h-10 rounded-xl bg-primary/10 border border-primary/15 flex items-center justify-center mx-auto mb-3 animate-pulse">
                        <span className="text-primary font-bold text-sm">F</span>
                    </div>
                    <p className="text-muted-foreground text-xs font-mono tracking-wider">Loading ForkGPT...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex h-[100dvh] bg-background flex-col md:flex-row overflow-hidden relative font-sans">
            <header className="md:hidden h-12 border-b border-border flex items-center justify-between px-3 bg-background shrink-0 relative z-30">
                <Sheet>
                    <SheetTrigger asChild>
                        <Button variant="ghost" size="icon" className="-ml-1 text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-md bg-primary/10 flex items-center justify-center">
                        <span className="text-primary font-bold text-[10px]">F</span>
                    </div>
                    <span className="font-semibold text-xs tracking-tight text-foreground">ForkGPT</span>
                </div>

                <Sheet>
                    <SheetTrigger asChild>
                        <Button variant="ghost" size="icon" className="-mr-1 text-muted-foreground">
                            <BranchIcon className="h-4 w-4" />
                        </Button>
                    </SheetTrigger>
                    <SheetContent side="right" className="p-0 w-[280px] border-l border-sidebar-border bg-sidebar">
                        <SheetTitle className="sr-only">Branch Tree</SheetTitle>
                        <GitTree
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-transparent z-10 overflow-hidden relative">
                <ChatView
                    messages={messages}
            ...
            <Dialog open={deleteConfirmation.isOpen} onOpenChange={(open) => setDeleteConfirmation(prev => ({ ...prev, isOpen: open }))}>
                <DialogContent className="bg-card border-border rounded-xl text-foreground sm:max-w-md p-0 overflow-hidden">
                    <div className="p-5">
                        <DialogHeader>
                            <DialogTitle className="text-lg font-bold tracking-tight">Delete {deleteConfirmation.type === "branch" ? "Branch" : "Chat"}</DialogTitle>
                            <DialogDescription className="text-muted-foreground text-sm mt-2 leading-relaxed">
                                Are you sure you want to delete <span className="text-foreground font-medium">&quot;{deleteConfirmation.name}&quot;</span>? This action is permanent.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter className="gap-2 mt-6">
                            <Button
                                ...
                                className="rounded-lg text-xs text-muted-foreground hover:text-foreground hover:bg-secondary flex-1"
                            >
                                Cancel
                            </Button>
                            <Button
                    ...
                    <div className="p-5">
                        <DialogHeader>
                            <DialogTitle className="text-lg font-bold tracking-tight flex items-center gap-2.5">
                                <div className="p-1.5 rounded-lg bg-[#a78bfa]/10 border border-[#a78bfa]/15">
                                    <MergeIcon className="h-4 w-4 text-[#a78bfa]/60" />
                                </div>
                                <span className="text-gradient-rose">Merge Branch</span>
                            </DialogTitle>
                            <DialogDescription className="text-muted-foreground text-sm mt-2 leading-relaxed">
                        ...
                        <DialogFooter className="gap-2 mt-6">
                            ...
                            <Button
                                ...
                                className="flex-1 rounded-lg text-xs font-semibold bg-secondary text-foreground hover:bg-accent border border-border"
                            >
                                Merge Branch
                            </Button>
                        </DialogFooter>
