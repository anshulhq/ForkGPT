...
import { db, messages, branches, type Message, type NewMessage } from "@/db";
...
async function ensureAuth() {
    try {
        ...
        let userId = (await cookieStore).get("flowgpt-userid")?.value;

        if (!userId) {
            userId = uuidv4();
            try {
                (await cookieStore).set("flowgpt-userid", userId, {
                    httpOnly: true,
